...
const MessageBoard = __webpack_require__(27);

ReactDOM.render(React.createElement(MessageBoard, { messages: window.__myMessages }), document.getElementById('content'));
...
class MessageBoard extends React.Component {
  ...
  componentDidMount() {
    fetch('/messages').then(response => response.json()).then(json => this.setState({ messages: json }));
  }
  ...
  saveMessage() {
    ...
    }).then(response => response.json()).then(json => json.ops[0]).then(newMessage => {
  ...
  render() {
    return React.createElement(
      ...
      React.createElement(
        'h1',
        null,
        'Northwestern Mutual Message Board'
      ),
      React.createElement(
        'div',
        { onKeyUp: (event => {
        ...
        React.createElement(
          ...
          React.createElement(
            ...
            React.createElement(
              'div',
              { className: 'large-4 columns md-text-field with-floating-label' },
              React.createElement('input', { type: 'text', placeholder: 'Enter your name:', value: this.state.username, onChange: this.handleUsernameChange, required: true })
            ),
            React.createElement(
              'div',
              { className: 'large-4 columns md-text-field with-floating-label' },
              React.createElement('input', { type: 'text', placeholder: 'Post your message here', value: this.state.messageText, onChange: this.handleTextChange, required: true })
            ),
            React.createElement(
              'div',
              { className: 'large-4 columns md-text-field with-floating-label' },
              React.createElement(
                'button',
                { className: 'button btn-cta', onClick: this.saveMessage },
                'Save'
              )
            )
          )
